int runCutflowByRegion_Sensitivity0419_reweighting(){
std::string savefilename = "Cutflow2LepTMVA_sensitivity0419_reweighting";
...
savefile << "|| r r r r r r ";
for(auto sig: sigma){
	savefile << " r" ;
}
savefile << " |} \n \\ensuremath{\\sqrt{s}=13 TeV}, \\ensuremath{\\mathcal{L}=140 fb^{-1}} "; 
for(std::string mcName : cutflowCategoryNames){
  if(mcName != "data") savefile << " & " << mcName.c_str();
}
savefile << "& Data & Data/MC & Total MC & Signal/BG & \\ensuremath{S/\\sqrt{B}} & Z";
for(auto sig: sigma){
...
for(int i = 1; i < histToShow->GetNbinsX(); i++){
  ...
  savefile << " & " << dataTotal << " & " << dataTotal/mcTotal << " & " << mcTotal << " & " << Signal/BG  << " & " << Signal/sqrt(BG) <<  " & " <<  Z;
  for(auto sig: sigma){
	  double sigmaBG = 0; 
	  double SoBwU = 0; 
	  double ZwU = 0;
